...
public class   AQS {
    ...
    public static void main(String[] args) {
        ...
        //
        // tryAcquire(arg)
        // 这个是方法是交由 继承AQS的具体实现同步器去重写该方法的
        // 如果尝试获取锁失败了
        // protected boolean tryAcquire(int arg) {
        //        throw new UnsupportedOperationException();
        // }
        //
        //   acquireQueued(addWaiter(Node.EXCLUSIVE), arg)   开始尝试去将当前线程加入到CLH等待队列中
        //  final boolean acquireQueued(final AbstractQueuedSynchronizer.Node node, int arg) {
        //        boolean interrupted = false;  // 当前线程的中断标志 先置为false
        //        try {
        //            for (;;) {
        //                final AbstractQueuedSynchronizer.Node p = node.predecessor();
                           // 获取当前线程CLH节点的前继节点
        //
        //                如果前继节点 是头节点并且再次尝试获取锁成功了
        ...
        //                    如果进入到这里就说头节点处理完成 CLH队列里就只有当前线程
        //                    setHead(node); // 把当前线程设置为头节点
        //                    p.next = null; // help GC  前置节点处理完毕 将前置节点的引用断开 方便gc回收
        //                    return interrupted; 返回不需要中断的标志
        //                }
                          // 到这里就说明确实需要老老实实排队 那就开始阻塞操作了
                          // 来看 shouldParkAfterFailedAcquire
        ...
        //        int ws = pred.waitStatus;  //主要是看前继节点的等待状态
        //        if (ws == AbstractQueuedSynchronizer.Node.SIGNAL)
        //            /* 前继节点已经设置好要求释放的信号 则当前线程可以开始阻塞了
